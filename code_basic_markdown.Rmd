...
```{r}
alt <- worldclim_global(var="elev", res=10, path=tempdir())
```
...
```{r}
map_thin <- read_csv("R_Icterus/I_data/I_10_2_thin1.csv") %>%
...
```{r}
inicial <- df_icter
thin1 <-  read_csv("R_Icterus/I_data/I_10_2_thin1.csv")
...
```{r}
set.seed(123)
icter_kmeans <- kmeans(df_kmeans, 4, nstart = 25)
