...
I14_gularis <- read_csv(here("R_Icterus", "I_data", "14_I_gularis_R.csv"))
limpio <- I14_gularis %>% select(-fid, -gbifID, -species, -month, -year)
...
png(file = "C:/Users/Yas/OneDrive/Maestría/proyecto/QGis/layers/Icterus_proyect/R_Icterus/I_data/graphics/I_14dendo.png", width = 5000, height = 2000)   
...
altura_max <- as.numeric(c(11.48502))
...
dendo <- as.dendrogram( clustering)
clusters <-cutree(dendo, k= 7, order_clusters_as_data = FALSE) 

#Opcional -----dendo <-rect.dendrogram(dendo, k = 21)


#opcional
plot(color_branches(dendo, k=7),leaflab="none") #genera un dendograma con color con el numero K de clusters indicado
...
table(clusters)  #imprime el numero de observaciones que integran cada cluster
...
cat(as.character(cluster6[1]), quote=FALSE,sep="\n")
cat(as.character(cluster4[1:76]), quote=FALSE,sep="\n")
cat(as.character(cluster3[1:405]), quote=FALSE,sep="\n")
cat(as.character(cluster5[1:324]), quote=FALSE,sep="\n")



cat(as.character(cluster1[1:900]), quote=FALSE,sep="\n")
cat(as.character(cluster1[901:1800]), quote=FALSE,sep="\n")
cat(as.character(cluster1[1801:2700]), quote=FALSE,sep="\n")
cat(as.character(cluster1[2701:3600]), quote=FALSE,sep="\n")
cat(as.character(cluster1[3601:4500]), quote=FALSE,sep="\n")
cat(as.character(cluster1[4501:5400]), quote=FALSE,sep="\n")
cat(as.character(cluster1[5401:6000]), quote=FALSE,sep="\n")
cat(as.character(cluster1[6001:6309]), quote=FALSE,sep="\n")
...
clust1 <- read.table(here("R_Icterus", "I_data", "14_cluster_1-5.txt"), header = FALSE)

#lectura de resultados de adelgazamiento
inicial <- read_csv(here("R_Icterus", "I_data", "14_I_gularis_R.csv"))
...
#arreglar formato
colnames(id_spthin)[1] <- "fid"


#guardar un csv
write_csv(id_spthin, "R_Icterus/I_data/14_cluster_spthin_1-5.csv")
...
I_spthin <- read_csv(here("R_Icterus", "I_data", "14_I_gularis_R.csv"))
numeros_eliminar <- scan(here("R_Icterus", "I_data", "eliminados__dendo_I14.txt"))
...
datos_ini_csv <- read_csv(here("R_Icterus", "I_data", "14_I_gularis_R.csv"))
datos_inicial <- nrow(datos_ini_csv)

datos_txt <- read.table(here("R_Icterus", "I_data", "eliminados__dendo_I14.txt"), header = FALSE)
...
png(file = "C:/Users/Yas/OneDrive/Maestría/proyecto/QGis/layers/Icterus_proyect/R_Icterus/I_data/graphics/I_14spthin.png", width = 666, height = 740)   
...
thin( loc.data = I_thin,
      ...
      out.dir = here("R_Icterus", "I_data", "output_I14_thin" ))
...
datos_fin_csv <- read_csv(here("R_Icterus", "I_data", "output_I14_thin","thinned_data_thin1.csv"))
...
#lectura de registros iniciales con id
thinned_data_thin1 <- read_csv(here("R_Icterus", "I_data","output_I14_thin", "thinned_data_thin1.csv"))
#lectura de resultados de adelgazamiento
inicial <- read_csv(here("R_Icterus", "I_data", "14_I_gularis_R.csv"))
...
#guardar un csv
write_csv(id_spthin, "R_Icterus/I_data/output_I14_thin/id_spthin.csv")
...
sthin <- read_csv(here("R_Icterus", "I_data", "output_I14_thin", "thinned_data_thin1.csv"))
